...
# Task References
advancedElicitationTask: '{project-root}/_byan/workflow/simple/advanced-elicitation/workflow.xml'
partyModeWorkflow: '{project-root}/_byan/workflow/simple/party-mode/workflow.md'
---

# STEP GOAL

Assemble the agent plan content into a complete Expert agent YAML file with sidecar folder structure. Expert agents require persistent memory storage, so the build creates a sidecar folder next to the agent.yaml (which gets installed to `_byan/memoire/` during BMAD installation).

## MANDATORY EXECUTION RULES

1. **EXPERT AGENT = SIDECAR REQUIRED**: Every Expert agent MUST have a sidecar folder created next to agent.yaml (build location), which will be installed to `_byan/memoire/` during BMAD installation
2. **CRITICAL_ACTIONS FORMAT**: All critical_actions MUST use `{project-root}/_byan/memoire/{sidecar-folder}/` for file operations (runtime path)
...
critical-actions:
  - name: '{action-name}'
    ...
    sidecar-files:
      - '{project-root}/_byan/memoire/{sidecar-folder}/{file1}.md'
      - '{project-root}/_byan/memoire/{sidecar-folder}/{file2}.md'
...
metadata:
  sidecar-folder: '{sidecar-folder-name}'
  sidecar-path: '{project-root}/_byan/memoire/{sidecar-folder}/'
  agent-type: 'expert'
...
1. **Create Sidecar Directory** (NEXT TO agent.yaml):
   ...
   - Note: This folder gets installed to `_byan/memoire/` during BMAD installation
...
3. **Add README to Sidecar**:
   ...
   `{project-root}/_byan/memoire/{sidecar-folder}/{filename}.md`
   ```
...
2. **Sidecar Structure**: Folder and files at `{agentBuildOutput}/{agent-name}-sidecar/` (build location, installs to `_byan/memoire/` during BMAD installation)
...
✅ critical_actions reference `{project-root}/_byan/memoire/{sidecar-folder}/` paths
